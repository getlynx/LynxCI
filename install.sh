...
while [ ! -O $listenerSer ] ; do # Only create the file if it doesn't already exist.
	...
	sleep 2
	sed -i 's/^[\t]*//' $listenerSer # Remove the pesky tabs inserted by the 'echo' outputs.
	sleep 2
...
while [ ! -O $lynxdSer ] ; do # Only create the file if it doesn't already exist.
	...
	sleep 2
	sed -i 's/^[\t]*//' $lynxdSer # Remove the pesky tabs inserted by the 'echo' outputs.
	sleep 2
...
install_lynx () {
	...
	" > /root/.lynx/lynx.conf
	sleep 2
	sed -i 's/^[\t]*//' /root/.lynx/lynx.conf # Remove the pesky tabs inserted by the 'echo' outputs.
